...
+
You could either retrieve the logs from the pod (`oc logs ostoy-frontend-5f6b9c9b67-w5cj4`) or use the `Deployment` to aggregate the logs (in which case you don't actually need to know the name of the pod).

. Run `oc logs deploy/ostoy-frontend` and you should see your messages:
+
[source,sh,role=execute]
----
